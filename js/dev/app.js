...
function initSliders() {
  ...
  if (document.querySelector(".pack__slider")) {
    new Swiper(".pack__slider", {
      // <- Вказуємо склас потрібного слайдера
      // Підключаємо модулі слайдера
      // для конкретного випадку
      modules: [Navigation],
  ...
  if (document.querySelector(".prod__slider")) {
  ...
  if (document.querySelector(".analys__slider")) {
    new Swiper(".analys__slider", {
      // <- Вказуємо склас потрібного слайдера
      // Підключаємо модулі слайдера
      // для конкретного випадку
      modules: [Navigation],
...
const langsBlock = document.querySelector(".actions-header__langs");
const langsList = document.querySelector(".actions-header__langs-list");
langsBlock.addEventListener("click", function(e) {
  e.stopPropagation();
  langsList.classList.toggle("actions-header__langs-list--active");
});
document.addEventListener("click", function(e) {
  if (!langsBlock.contains(e.target)) {
    langsList.classList.remove("actions-header__langs-list--active");
  }
});
class DynamicAdapt {
  constructor() {
...
const observer = new IntersectionObserver(
  (entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        const span = entry.target.querySelector(".num");
        if (span) animateNumber(span);
...
document.querySelectorAll(".item-hero").forEach((item) => observer.observe(item));
document.querySelectorAll(".item-about").forEach((item) => observer.observe(item));
document.querySelectorAll(".reviews__video").forEach((wrap) => {
  const video = wrap.querySelector(".reviews__player");
  const playBtn = wrap.querySelector(".custom-play-btn");
  const nameEl = wrap.querySelector(".reviews__name");
  if (!video || !playBtn || !nameEl) return;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const popupService = document.getElementById("popupServiceName");
  const popupClose = popup.querySelector(".popup__close");
  const overlay = popup.querySelector(".popup__overlay");
  const buttons = document.querySelectorAll(
    ".pack__btn, .prod__button, .content__btn, .analys__button, .item-services__button, .actions-header__button"
  );
  buttons.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      let category = "";
      let tariff = "";
      const card = btn.closest(".pack__slide, .prod__slide, .content__slide, .analys__slide");
      if (card) {
        tariff = card.querySelector("h2")?.textContent.trim() || "";
        if (card.closest(".pack")) category = "Пакети Послуг";
        if (card.closest(".prod")) category = "Продакшн";
        if (card.closest(".content")) category = "Контент Ферма";
        if (card.closest(".analys")) category = "Комплексний Аналіз Бренду";
      }
      if (btn.closest(".item-services")) {
        const servicesCard = btn.closest(".item-services");
        tariff = servicesCard.querySelector(".item-services__title")?.textContent.trim() || "";
        category = "Безкоштовна консультація";
      }
      if (btn.classList.contains("actions-header__button")) {
        category = "Отримати пропозицію";
        tariff = "Консультація";
      }
      popupCategory.textContent = category;
      popupService.textContent = tariff;
